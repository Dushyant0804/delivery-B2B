...
// Primary Sorting never runs dimension checks or the chute-full check
// (see plugins/primarySortEngine.js) — only client-rejects, barcode/no-
// find rejects, and LMM (no matching rule) are actually reachable here.
const REASON_OPTIONS = ["dnf", "dbo", "ibo", "hv", "unx", "rej", "ar", "nsz", "lmm"];

// ─── Main component ───────────────────────────────────────────────────────────

const PrimarySortingReport = () => {
  ...
  const [bagCodeFilter,  setBagCodeFilter ] = useState("");
  ...
  // Date range (filters on primary_scantime)
  ...
  const fetchData = useCallback(async (overrides = {}) => {
    ...
    try {
      ...
      const res = await axios.get(`${API}/primary-sorting`, { params: p });
  ...
  }, [page, limit, appliedSearch, dateRange, searchTrigger, usernameFilter, bagCodeFilter, bayFilter, reasonFilter]);
  ...
  const exportData = () => {
    ...
    if (bagCodeFilter)  params.append("bag_code", bagCodeFilter);
    if (bayFilter)      params.append("bay_id", bayFilter);
    if (reasonFilter)   params.append("reason", reasonFilter);
    window.open(`${API}/primary-sorting/export?${params.toString()}`, "_blank");
  };
  ...
  const chips = [
    ...
    bagCodeFilter   && { key: "bag",      label: "Bag Code", value: bagCodeFilter,   clear: () => { setBagCodeFilter(""); setPage(1); } },
    bayFilter       && { key: "bay",      label: "Bay ID",   value: bayFilter,       clear: () => { setBayFilter(""); setPage(1); } },
    reasonFilter    && { key: "reason",   label: "Reason",   value: reasonFilter.toUpperCase(), clear: () => { setReasonFilter(""); setPage(1); } },
  ...
  const outcomeBadge = (r) => {
    const isSorted = !r.reason;
    return (
      <span className={`pr-badge ${isSorted ? "pr-badge-sorted" : "pr-badge-rej"}`}>
        {isSorted ? "SORTED" : "REJECTED"}
      </span>
    );
  };
      ...
      <div className="pr-card">
        ...
        <div className="pr-page-header">
          <div className="pr-page-header__left">
            <h2 className="pr-page-title">Primary Sorting Report</h2>
        ...
        <div className="pr-filter-card">
          ...
          <div className="pr-filter-row">
            ...
            <div className="pr-field-group">
              <label className="pr-field-label">WBN</label>
            ...
            <div className="pr-field-group">
              <label className="pr-field-label">Bag Code</label>
              <div className="pr-field-input-wrap">
                <input
                  className="pr-field-input"
                  value={bagCodeFilter}
                  onChange={e => { setBagCodeFilter(e.target.value); setPage(1); }}
                  placeholder="e.g. D003"
                />
                {bagCodeFilter && <button className="pr-field-clear" onClick={() => { setBagCodeFilter(""); setPage(1); }}>✕</button>}
              </div>
            </div>
            ...
            <div className="pr-field-group">
              ...
              <select className="pr-field-input pr-field-select"
                value={reasonFilter} onChange={e => { setReasonFilter(e.target.value); setPage(1); }}>
        ...
        <DataTable
          ...
          className="pr-table"
          emptyMessage="No primary sorting scans found for the selected filters."
        ...
        >
          ...
          <Column header="Status"  body={outcomeBadge} style={{ minWidth: 100 }} />
          <Column
            header="Bag Code"
            body={r => r.bag_code ? <span className="pr-bag-tag">{r.bag_code}</span> : <span className="pr-cell-muted">—</span>}
            style={{ minWidth: 110 }}
          />
          <Column
            header="Bay ID"
          ...
          <Column
            header="Reason"
            body={r => r.reason ? <span className="pr-reason-tag">{r.reason.toUpperCase()}</span> : <span className="pr-cell-muted">—</span>}
            style={{ minWidth: 90 }}
          />
          <Column header="Scan Time (IST)" body={r => fmtIST(r.primary_scantime)} style={{ minWidth: 170 }} />
        </DataTable>
...
export default PrimarySortingReport;