...
import "../styles/SortedPayloadsReport.css";
...
function SpinBox({ value, min, max, onChange, color, width = 52 }) {
  ...
  return (
    <div className="sp-spb" style={{ width }} tabIndex={0}
      onKeyDown={e => { if (e.key === "ArrowUp") { e.preventDefault(); inc(); } if (e.key === "ArrowDown") { e.preventDefault(); dec(); } }}>
      <button className="sp-spb__arrow" onClick={inc} tabIndex={-1}>▲</button>
      <div className="sp-spb__val" style={{ color, borderColor: color + "80" }}>{p(value)}</div>
      <button className="sp-spb__arrow" onClick={dec} tabIndex={-1}>▼</button>
    </div>
  );
}
...
function DateTimePicker({ label, color, value, onChange }) {
  ...
  return (
    <div className="sp-dtp">
      <div className="sp-dtp__label" style={{ color }}>
        <span className="sp-dtp__dot" style={{ background: color }} />
        {label}
      </div>
      <div className="sp-dtp__body">
        <div className="sp-dtp__cal">
          <div className="sp-dtp__nav">
            <button className="sp-dtp__navbtn" onClick={prevM}>‹</button>
            <span className="sp-dtp__navtitle">{MONTHS_SHORT[view.month]} {view.year}</span>
            <button className="sp-dtp__navbtn" onClick={nextM}>›</button>
          </div>
          <div className="sp-dtp__dayhdr">
            {DAYS_SHORT.map(d => <span key={d}>{d}</span>)}
          </div>
          <div className="sp-dtp__grid">
            {cells.map((d, i) => (
              <button key={i} disabled={!d}
                className={["sp-dtp__cell",
                  !d       ? "sp-dtp__cell--blank" : "",
                  isSel(d) ? "sp-dtp__cell--sel"   : "",
                  isToday(d) && !isSel(d) ? "sp-dtp__cell--today" : "",
                ].join(" ")}
        ...
        <div className="sp-dtp__time">
          <div className="sp-dtp__time-title">TIME</div>
          <div className="sp-dtp__time-hint">24h IST</div>
          <div className="sp-dtp__spinrow">
            <SpinBox value={sel.hour}   min={0} max={23} onChange={h => emit({ ...sel, hour: h })}   color={color} />
            <span className="sp-dtp__colon">:</span>
            <SpinBox value={sel.minute} min={0} max={59} onChange={m => emit({ ...sel, minute: m })} color={color} />
          </div>
          <div className="sp-dtp__timedisp" style={{ color, borderColor: color + "50" }}>
            {String(sel.hour).padStart(2, "0")}:{String(sel.minute).padStart(2, "0")}
          </div>
          <div className="sp-dtp__datedisp">
...
function FilterDrawer({ open, onClose, dateRange, onApplyDate, onQuickSelect, activeLabel }) {
  ...
  return (
    <>
      <div className={`sp-drawer-backdrop ${open ? "sp-drawer-backdrop--open" : ""}`} onClick={onClose} />
      <div className={`sp-filter-drawer ${open ? "sp-filter-drawer--open" : ""}`}>

        <div className="sp-fd-header">
          <div className="sp-fd-header__left">
            <span style={{ fontSize: 20 }}>🔍</span>
            <div>
              <div className="sp-fd-header__title">Date &amp; Time Filter</div>
              <div className="sp-fd-header__sub">All times in IST</div>
            </div>
          </div>
          <button className="sp-fd-close" onClick={onClose}>✕</button>
        </div>

        <div className="sp-fd-tabs">
          <button className={`sp-fd-tab ${tab === "quick"  ? "sp-fd-tab--active" : ""}`} onClick={() => setTab("quick")}>⚡ Quick</button>
          <button className={`sp-fd-tab ${tab === "custom" ? "sp-fd-tab--active" : ""}`} onClick={() => setTab("custom")}>🗓 Custom</button>
        </div>

        <div className="sp-fd-body">
          {tab === "quick" && (
            <div>
              <p className="sp-fd-section-label">Select preset range</p>
              <div className="sp-fd-quick-grid">
                {QUICK_RANGES.map(r => {
                  const isActive = activeLabel === r.label;
                  return (
                    <button key={r.label}
                      className={`sp-fd-preset ${isActive ? "sp-fd-preset--active" : ""}`}
                      onClick={() => { onQuickSelect(r.label, r.getRange()); onClose(); }}
                    >
                      <span style={{ fontSize: 15 }}>{r.icon}</span>
                      <span className="sp-fd-preset__label">{r.label}</span>
                      {isActive && <span className="sp-fd-preset__check">✓</span>}
                    </button>
                  );
                })}
              </div>
          ...
          {tab === "custom" && (
            <div className="sp-fd-custom">
              <DateTimePicker label="Start" color="#D71920" value={start} onChange={setStart} />
              <div className="sp-fd-divider" />
              <DateTimePicker label="End"   color="#1F2937" value={end}   onChange={setEnd} />
              {diffMs > 0
                ? <div className="sp-fd-duration">⏱ Duration: <b>{hrs}h {mins}m</b></div>
                : start && end && <div className="sp-fd-duration sp-fd-duration--warn">⚠️ End must be after Start</div>
              }
              <button className="sp-fd-apply-btn"
        ...
        <div className="sp-fd-footer">
          <div className="sp-fd-footer__label">Currently showing</div>
          <div className="sp-fd-footer__range">
            {activeLabel
              ? <span className="sp-fd-footer__badge">{activeLabel}</span>
              : dateRange.start
...
function FilterChip({ label, value, onRemove }) {
  ...
  return (
    <span className="sp-chip">
      ...
      <span className="sp-chip__val">{value}</span>
      <button className="sp-chip__x" onClick={onRemove}>✕</button>
    </span>
  );
}
...
const SortedPayloadsReport = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API}/sorted-payloads-report`, { params: p });

      if (!res.data || typeof res.data.total !== "number" || !Array.isArray(res.data.rows)) {
        throw new Error("Unexpected response from /sorted-payloads-report");
      }
  ...
  const exportData = () => {
    ...
    window.open(`${API}/sorted-payloads-report/export?${params.toString()}`, "_blank");
  };
  ...
  return (
    <div className="sp-root">
      ...
      <div className="sp-card">
        ...
        <div className="sp-page-header">
          <div className="sp-page-header__left">
            <h2 className="sp-page-title">Sorted Payloads</h2>
            {!wbnInput && badgeText && <span className="sp-range-badge">{badgeText}</span>}
            {wbnInput && <span className="sp-range-badge sp-range-badge--search">🔍 "{wbnInput}"</span>}
          </div>
          <span className="sp-total-count">{total.toLocaleString()} records</span>
        </div>

        <div className="sp-filter-card">
          <div className="sp-filter-row">
            <div className="sp-field-group">
              <label className="sp-field-label">WBN</label>
              <div className="sp-field-input-wrap">
                ...
                />
                {wbnInput && <button className="sp-field-clear" onClick={() => setWbnInput("")}>✕</button>}
              </div>
            </div>
          </div>

          <div className="sp-filter-actions">
            <button className="sp-action-btn sp-action-btn--export" onClick={exportData}>⬇ Export CSV</button>
            <button
              className={`sp-action-btn sp-action-btn--date ${drawerOpen ? "sp-action-btn--date-active" : ""}`}
              onClick={() => setDrawerOpen(true)}
            >
              📅 Date Filter
            </button>
            <button className="sp-action-btn sp-action-btn--reset" onClick={handleReset}>↺ Reset</button>
            <button className="sp-action-btn sp-action-btn--search"
              onClick={() => {
                if (wbnInput.length >= 2) {
                  setAppliedSearch(wbnInput);
                  setDateRange({ start: null, end: null });
        ...
        {chips.length > 0 && (
          <div className="sp-chips">
            ...
            <button className="sp-chip-clear-all" onClick={handleReset}>Clear all</button>
          </div>
        )}
        ...
        <DataTable
          ...
          loading={loading}
          className="sp-table"
          emptyMessage="No sorted payloads found."
          scrollable
          scrollHeight="55vh"
          resizableColumns
        ...
        >
          ...
          <Column
            header="Chute ID"
            body={r => r.chute_id ? <span className="sp-tag">{r.chute_id}</span> : <span className="sp-cell-muted">—</span>}
            style={{ minWidth: 90 }}
          />
          <Column
            header="City"
            body={r => r.city || <span className="sp-cell-muted">—</span>}
            style={{ minWidth: 110 }}
          />
          <Column
            header="Zone"
            body={r => r.zone || <span className="sp-cell-muted">—</span>}
            style={{ minWidth: 90 }}
          />
          <Column
            header="MOT"
            body={r => r.mot || <span className="sp-cell-muted">—</span>}
            style={{ minWidth: 80 }}
          />
          <Column
            header="Product Type"
            body={r => r.pdt || <span className="sp-cell-muted">—</span>}
            style={{ minWidth: 110 }}
          />
          <Column
            header="Client"
            body={r => r.client_name || <span className="sp-cell-muted">—</span>}
            style={{ minWidth: 130 }}
          />
          <Column
            header="Payload"
            body={r => (
              <button className="sp-json-btn" onClick={() => openJson(r.payload, r.wbn)}>👁 View</button>
            )}
            style={{ minWidth: 90 }}
          />
          <Column header="Updated (IST)" body={r => fmtIST(r.updated_at)} style={{ minWidth: 165 }} />
        </DataTable>

        <div className="sp-pagination">
          <span className="sp-pagination__label">displaying page</span>
          <button className="sp-page-btn" disabled={page === 1} onClick={() => setPage(1)}>First</button>
          <button className="sp-page-btn sp-page-btn--arrow" disabled={page === 1} onClick={() => setPage(p => Math.max(1, p - 1))}>‹</button>
          ...
          <button className="sp-page-btn sp-page-btn--arrow" disabled={page === totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))}>›</button>
          <button className="sp-page-btn" disabled={page === totalPages} onClick={() => setPage(totalPages)}>Last</button>
        </div>
      ...
      {jsonDialog && (
        <div className="sp-json-overlay" onClick={() => setJsonDialog(false)}>
          <div className="sp-json-modal" onClick={e => e.stopPropagation()}>
            <div className="sp-json-modal__header">
              <span>Payload — {jsonWbn}</span>
              <button onClick={() => setJsonDialog(false)}>✕</button>
            </div>
            <pre className="sp-json-box">
...
export default SortedPayloadsReport;